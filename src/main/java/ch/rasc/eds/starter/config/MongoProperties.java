...
public class MongoProperties {
	 ...
	 */
	public MongoClient createMongoClient(MongoClientOptions opts, Environment environment)
			throws UnknownHostException {
		try {
			MongoClientOptions options = opts;
			if (hasCustomAddress() || hasCustomCredentials()) {
				...
				if (hasCustomCredentials()) {
					String _database = this.authenticationDatabase == null
							? getMongoClientDatabase() : this.authenticationDatabase;
					credentials = Arrays.asList(MongoCredential.createMongoCRCredential(
							this.username, _database, this.password));
				}
				else {
					credentials = Collections.emptyList();
				}
				String _host = this.host == null ? "localhost" : this.host;
				int _port = determinePort(environment);
				return new MongoClient(Arrays.asList(new ServerAddress(_host, _port)),
						credentials, options);
			}
